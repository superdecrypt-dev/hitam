...
api_create_account() {
  ...
  # Kalau quota > 0 langsung tulis ke quota.db (logicnya bisa copy dari aksi_buat)
  # ...

  # Generate link & file txt + html (reuse logic dari aksi_buat)
  # render_account_html "$proto" "$user" "$domain" "$path" "$secret" "$exp" "$created" "$link_tls" "$link_nontls"
  # rebuild_web_index

  echo '{"ok":true,"user":"'"$user"'","proto":"'"$proto"'","expired":"'"$exp"'"}'
}
