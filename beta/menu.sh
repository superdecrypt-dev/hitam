...
api_create_account() {
  # Format:
  # menu --api-create --proto vmess --user foo --days 30 --quota-gb 0
  shift 1
  local proto="" user="" days="30" quota_gb="0"
  ...
  if [[ -z "$proto" || -z "$user" ]]; then
    echo '{"ok":false,"error":"missing proto or user"}'
    return 1
  fi

  # Validasi sama seperti di aksi_buat
  if username_exists_any_proto "$user"; then
    echo '{"ok":false,"error":"username already exists"}'
    return 1
  fi
  ...
  if [[ -z "$domain" || -z "$path" ]]; then
    echo '{"ok":false,"error":"domain or path not set"}'
    return 1
  fi

  # Generate secret sesuai proto (pakai code yg sudah ada di aksi_buat)
  local secret=""
  case "$proto" in
    vmess|vless) secret="$("$XRAY_BIN" uuid)";;
    trojan|http|socks) secret="$(tr -dc 'a-zA-Z0-9' < /dev/urandom | head -c 16 || true)";;
    shadowsocks) secret="$(openssl rand -base64 16)";;
    *) echo '{"ok":false,"error":"invalid proto"}'; return 1;;
  esac
  ...
  # Simpan ke ledger (+ kolom quota di posisi ke-5)
  local quota_bytes=0
  if [[ "$quota_gb" =~ ^[0-9]+$ ]]; then
    quota_bytes=$((quota_gb * 1024 * 1024 * 1024))
  fi
  ledger_add "$proto" "$user" "$secret" "$exp" "$created" "$quota_bytes"

  # Kalau quota > 0 → daftarkan ke quota.db (simple versi)
  if (( quota_bytes > 0 )); then
    local tmpq="$QUOTA_DB.tmp.$$"
    grep -v "^$user|" "$QUOTA_DB" 2>/dev/null > "$tmpq" || true
    # mulai dari used=0, last_up=0, last_down=0 (user baru)
    echo "$user|$quota_bytes|0|0|0" >> "$tmpq"
    mv "$tmpq" "$QUOTA_DB"
  fi

  # Generate link seperti di aksi_buat
  local link_tls="" link_nontls="" notes=""
  case "$proto" in 
    vmess) 
      link_tls="$(vmess_link_tls "$secret" "$domain" "$path" "$user")"
    ...
    http)
      link_tls="$(http_link_tls "$secret" "$domain" "$user" "$path")"
      link_nontls="$(http_link_nontls "$secret" "$domain" "$user" "$path")"
      notes="Gunakan aplikasi Exclave, network WS, path /$path"
      ;;
    socks)
      link_tls="$(socks_link_tls "$secret" "$domain" "$user" "$path")"
      link_nontls="$(socks_link_nontls "$secret" "$domain" "$user" "$path")"
...
EOF
  ...
  # Generate HTML + update index
  render_account_html "$proto" "$user" "$domain" "$path" "$secret" "$exp" "$created" "$link_tls" "$link_nontls"
  rebuild_web_index

  local panel_url="https://${domain}/panel/accounts/${proto}/${user}.html"

  printf '{"ok":true,"user":"%s","proto":"%s","expired":"%s","panel_url":"%s","link_tls":"%s","link_nontls":"%s"}\n' \
    "$user" "$proto" "$exp" "$panel_url" "$link_tls" "$link_nontls"
}

api_delete_account() {
  # Format:
  # menu --api-delete --proto vmess --user foo
  shift 1
  local proto="" user=""
  ...
  if [[ ! -f "$db" ]] || ! grep -q "^$user|" "$db"; then
    echo '{"ok":false,"error":"user not found"}'
    return 1
  fi
  ...
  # Hapus halaman HTML + update index
  delete_account_html "$proto" "$user"
  rebuild_web_index
...
api_extend_account() {
  # Format:
  # menu --api-extend --proto vmess --user foo --days 30
  shift 1
  local proto="" user="" days="0"
  ...
  if [[ -z "$proto" || -z "$user" || -z "$days" ]]; then
    echo '{"ok":false,"error":"missing proto/user/days"}'
    return 1
  fi
  if [[ ! "$days" =~ ^[0-9]+$ ]]; then
    echo '{"ok":false,"error":"days must be number"}'
    return 1
  fi

  local db="$ACCOUNTS_DIR/$proto.db"
  if [[ ! -f "$db" ]]; then
    echo '{"ok":false,"error":"db not found"}'
    return 1
  fi
  ...
  local oldexp
  oldexp=$(echo "$line" | cut -d'|' -f3)

  # Perpanjang
  if ! ledger_extend "$proto" "$user" "$days"; then
    echo '{"ok":false,"error":"extend failed"}'
    return 1
  fi

  restart_xray

  # Ambil tanggal expired baru
  local newline newexp
  newline=$(grep "^$user|" "$db" || true)
  newexp=$(echo "$newline" | cut -d'|' -f3)

  # Update HTML + index
  refresh_account_html "$proto" "$user"
  rebuild_web_index

  printf '{"ok":true,"user":"%s","proto":"%s","expired_old":"%s","expired_new":"%s"}\n' \
    "$user" "$proto" "$oldexp" "$newexp"
}

api_list_accounts_json() {
  # Format:
  # menu --api-list-json
  local tmp
  tmp=$(mktemp)
  ...
  for p in vmess vless trojan shadowsocks http socks; do
    ...
    while IFS='|' read -r u s e c q; do
      ...
      local quota_int="${q:-0}"
      local quota_gb=0
      if [[ -n "$quota_int" && "$quota_int" -gt 0 ]] 2>/dev/null; then
        quota_gb=$((quota_int / 1024 / 1024 / 1024))
      fi
      ...
      jq -nc \
        ...
        --arg created "${created:-"-"}" \
        --arg expired "${expired:-"-"}" \
        --argjson quota_bytes "${quota_int:-0}" \
        --argjson quota_gb "$quota_gb" \
        '{proto:$proto,user:$user,created:$created,expired:$expired,quota_bytes:$quota_bytes,quota_gb:$quota_gb}' \
        >> "$tmp"
      have_any=1
    done < "$db"
  done
